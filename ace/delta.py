...
SUPPORTED_METADATA_KEYS = {"helpful", "harmful", "neutral"}


def _coerce_supported_metadata(payload: object) -> Dict[str, int]:
    if not isinstance(payload, dict):
        return {}
    metadata: Dict[str, int] = {}
    for key, value in payload.items():
        normalized_key = str(key)
        if normalized_key not in SUPPORTED_METADATA_KEYS:
            continue
        try:
            metadata[normalized_key] = int(value)
...
class DeltaOperation:
    ...
    def from_json(cls, payload: Dict[str, object]) -> "DeltaOperation":
        return cls(
            ...
            metadata=_coerce_supported_metadata(payload.get("metadata")),
        )
