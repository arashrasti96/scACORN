...
#from .prompts_cell_v0 import CURATOR_PROMPT, GENERATOR_PROMPT, REFLECTOR_PROMPT

def _safe_json_loads(text: str) -> Dict[str, Any]:
    def _sanitize_response(payload: str) -> str:
        ...
        if trimmed and trimmed[0] != "{":
            ...
            if start != -1 and end != -1 and end > start:
                trimmed = trimmed[start : end + 1].strip()
        return trimmed

    sanitized = _sanitize_response(text)

    def _log_failure(original_exc: Exception) -> None:
        ...
        with debug_path.open("a", encoding="utf-8") as fh:
            ...
            fh.write("\n")
            fh.write("sanitized: ")
            fh.write(repr(sanitized))
            fh.write("\n")
        raise ValueError(f"LLM response is not valid JSON: {original_exc}\n{text}") from original_exc

    parsers = [
        ("json", lambda payload: json.loads(payload)),
        ("python_literal", lambda payload: ast.literal_eval(payload)),
    ]
    if yaml is not None:
        parsers.append(("yaml", lambda payload: yaml.safe_load(payload)))

    last_exc: Optional[Exception] = None
    data: Any = None
    for _, parser in parsers:
        try:
            data = parser(sanitized)
            break
    ...
    if not isinstance(data, dict):
        _log_failure(last_exc or ValueError("Expected a JSON object from LLM."))
...
class GeneratorOutput:
    ...
    final_answer: str
    bullet_ids: List[str]
    bullet_ids_used_from_playbook: List[str]
    raw: Dict[str, Any]
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                bullet_ids_used = [
                    str(item)
                    for item in data.get("bullet_ids_used_from_playbook", bullet_ids)
                    if isinstance(item, (str, int))
                ]
                return GeneratorOutput(
                    ...
                    bullet_ids_used_from_playbook=bullet_ids_used,
                    raw=data,
                )
